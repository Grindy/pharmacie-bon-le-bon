  ...
  return (
    <main className="flex justify-center items-center py-20">
      <div className="w-full max-w-md bg-[var(--card)] p-8 rounded-[var(--radius)] shadow-[var(--shadow)] border border-[var(--accent)]">
        <h1 className="text-3xl font-bold mb-6 text-center text-[var(--accent-dark)]">Créer un compte</h1>

        <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
          {/* Courriel */}
          <div className="flex flex-col">
            <label className="mb-1 font-medium text-[var(--label)]">Courriel</label>
            <input name="courriel" type="email" placeholder="exemple@courriel.com" className="border rounded px-3 py-2 placeholder-[var(--placeholder)] inputlight" required />
          </div>

          {/* Pseudo */}
          <div className="flex flex-col">
            <label className="mb-1 font-medium text-[var(--label)]">Pseudo</label>
            <input name="pseudonyme" type="text" placeholder="Votre nom d'utilisateur" className="border rounded px-3 py-2 placeholder-[var(--placeholder)] inputlight" required />
          </div>

          {/* Mot de passe */}
          <div className="flex flex-col">
            <label className="mb-1 font-medium text-[var(--label)]">Mot de passe</label>
            <input name="motDePasse" type="password" placeholder="Votre mot de passe" className="border rounded px-3 py-2 placeholder-[var(--placeholder)] inputlight" required />
          </div>

          {/* Erreur */}
          {msgErreur && <p className="text-red-600 text-sm">{msgErreur}</p>}
